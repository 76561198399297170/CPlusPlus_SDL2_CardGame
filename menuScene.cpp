...
MenuScene::MenuScene()
{
    ...
    this->m_bk_sky->setPosition({ 0.0f, 0.0f });
    ...
    this->m_bk_sky->addFrameChangedAction([this]()
        {
            static int move_pos = 10;
            static int dir = -1;
            this->m_bk_sky->m_dst_rect.x += dir, move_pos -= dir;
            if (move_pos == 0 || move_pos == 2500) dir *= -1;
        });
    this->m_bk_sky->play();
    ...
    this->m_bk_sun->setPosition({ 0.0f, 0.0f });
    ...
    this->m_bk_plc->setPosition({ 0.0f, 0.0f });
...
void MenuScene::exit()
{
    Mix_FadeOutChannel(this->m_music_cannel, 2000);
}
